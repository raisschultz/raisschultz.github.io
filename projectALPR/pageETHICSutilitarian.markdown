---
layout: page
navbar-links:
  Homepage: "https://raisschultz.github.io/"
  Project Introduction: "https://raisschultz.github.io/projectALPR/introduction/"
  Discussion of the Technology: 
    - Benefits: "https://raisschultz.github.io/projectALPR/benefits/"
    - Legal Concerns and Implications: "https://raisschultz.github.io/projectALPR/legalconcerns/"
    - Social and Political Concerns and Implications: "https://raisschultz.github.io/projectALPR/socialethicalconcerns/"
    - Technological Concerns and Implications: "https://raisschultz.github.io/projectALPR/technologicalconcerns/"
    - Proposed Changes and Improvements: "https://raisschultz.github.io/projectALPR/proposedchanges/"
  Technology Examples:
    - Flock Cameras: "https://raisschultz.github.io/projectALPR/flockalprs/"
    - Border Patrol Use: "https://raisschultz.github.io/projectALPR/borderpatrolalprs/"
  Ethical Perspectives:
    - A Deontological Perspective on ALPRs: "https://raisschultz.github.io/projectALPR/deontologicallens"
  Bibliography: "https://raisschultz.github.io/projectALPR/bibliography/"
title: A Utilitarian Perspective
permalink: /projectALPR/utilitarianlens/
---
# Government - Law, Order, and National Security

From the ethical perspective of the government, and law enforcement, ALPRs are justified as necessary tools that increase efficiency, and advance public safety and national security. This ethical perspective is grounded in the idea that the state has an ethical duty to prevent crime and maintain social order.<sup id="fn1_ref"><a href="#fn1">1</a></sup> The use of more efficient tools like ALPRs helps them fulfill their responsibility to the public. 

This ethical framing is rooted in utilitarian reasoning, which evaluates actions based on their outcomes and prioritizes what produces the greatest overall benefit for society.<sup id="fn2_ref"><a href="#fn2">2</a></sup> With this perspective, ALPRs are ethically acceptable because they reduce crime, enhance investigative capabilities, and enable faster, more accurate identification of vehicles associated with theft, trafficking, smuggling, and other serious criminal activity. Any intrusion into personal privacy is considered minimal compared to the increases in public safety these systems might provide.

The Department of Homeland Security (DHS) describes ALPRs as operationally essential technologies that simply automate tasks officers have always performed manually.<sup id="fn3_ref"><a href="#fn3">3</a></sup> Capturing license plates automatically is simply digitization of information that is already publicly visible on the roads. Automating this process is an efficiency upgrade that strengthens the government’s capacity to protect communities. 

ALPRs and their databases are force multipliers for local law enforcement that allow them to focus on tasks and parts of the job that demand real human work, like investigations, community engagement, and emergencies. This logic can be extended onto the national level. CBP and ICE argue that access to nationwide ALPR networks like Flock’s helps them identify vehicles connected to crime and trafficking, increasing officer safety and efficiency, aiding in what these organizations see as their ethical duty; public and national security.<sup id="fn4_ref"><a href="#fn4">4</a></sup> 

![ValueOfALPRs](https://raisschultz.github.io/assets/img/utilitarian.png)
<sub>(Value of ALPRs in Safety and Security, Allerin)</sub>

From the ethical perspective of law enforcement agencies and the government, the use of ALPR technologies is necessary, and justified. ALPRs and networks like Flock’s contribute to more secure communities and a more secure nation, which fulfills the government's obligation to protect its population.

Footnotes  

<sup id="fn1">1 Internet Encyclopedia of Philosophy, “Social Contract Theory,” https://iep.utm.edu/soc-cont/</sup>  
<sup id="fn2">2 Internet Encyclopedia of Philosophy, “Act and Rule Utilitarianism,” https://iep.utm.edu/util-a-r/</sup>  
<sup id="fn3">3 U.S. Department of Homeland Security, Privacy Impact Assessment for CBP License Plate Reader Technology (PIA-049(a)) (July 2020), https://www.dhs.gov/sites/default/files/publications/privacy-pia-cbp049a-cbplprtechnology-july2020.pdf</sup>  
<sup id="fn4">4 Associated Press, “U.S. Border Patrol is monitoring U.S. drivers and detaining those with ‘suspicious’ travel patterns,” 2025, https://apnews.com/article/immigration-border-patrol-surveillance-drivers-ice-trump-9f5d05469ce8c629d6fecf32d32098cd</sup>  


